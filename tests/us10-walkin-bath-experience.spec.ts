...
test.describe("US10: Walk-In Bath User Experience", () => {
  test("TC01: Reviews section is visible on page scroll", async ({
    reviewsSection,
  }) => {
    await reviewsSection.scrollToReviews();
    await expect(reviewsSection.isReviewsVisible()).resolves.toBe(true);
    // Validate meaningful content, not just presence of the container
    expect(await reviewsSection.getHeadingText()).toContain(
      "See What People Are Saying",
    );
    expect(await reviewsSection.getReviewCount()).toBeGreaterThan(0);
  });

  test('TC02: Click "Show more" => hidden review comments become visible', async ({
    reviewsSection,
  }) => {
    await reviewsSection.scrollToReviews();
  ...
  test("TC03: Click customer's real installation photo in Denny's review => photo opens in zoomed/lightbox overlay (DEF-05)", async ({
    reviewsSection,
  }) => {
